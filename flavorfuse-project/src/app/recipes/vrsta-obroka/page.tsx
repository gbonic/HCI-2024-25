  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        // Filtriraj recepte samo za kategoriju "Prema vrsti obroka"
        const zdravRecepti = combinedRecipes.filter((recipe) =>
          recipe.fields.kategorija?.includes("Prema vrsti obroka")
        );
        ...
        // Dohvati podkategorije samo za "Prema vrsti obroka"
        const zdravSubcategories = podkategorijeResponse.items
          .filter((podkat: any) => {
            const isMatch = podkat.fields.kategorija?.fields.nazivKategorije === "Prema vrsti obroka";
            console.log(`Podkategorija: ${podkat.fields.nazivPodkategorije}, Kategorija: ${podkat.fields.kategorija?.fields.nazivKategorije}, Matches: ${isMatch}`);
            return isMatch;
          })
          .map((podkat: any) => podkat.fields.nazivPodkategorije || "");

        console.log("Zdrave podkategorije:", zdravSubcategories);
